...
import { saveDbAttachment, useDbAttachmentStorage } from "@/lib/attachment-store";
...
export async function POST(req: NextRequest) {
  ...
  if (!useDbAttachmentStorage) await mkdir(uploadDir, { recursive: true });
  const saved = [];
  for (const file of files) {
    if (file.size > MAX_FILE_SIZE) return jsonError(`${file.name} is larger than 25MB`, 400, "FILE_TOO_LARGE");
    if (useDbAttachmentStorage) {
      const storedPath = await saveDbAttachment(String(user._id), { name: file.name, mimeType: file.type, content: Buffer.from(await file.arrayBuffer()) });
      saved.push({ name: file.name, size: file.size, mimeType: file.type, path: storedPath });
      continue;
    }
